...
public class GameObj {
	...
	public static void moveTails(int i, int x, int y, List<GameObj> tails, Direction dir) {
		if (dir == Direction.LEFT) {
			if (Math.abs(x - GameCourt.snakeList.get(i).getX()) > 10 || Math.abs(y - GameCourt.snakeList.get(i).getY()) > 10) {
				GameCourt.snakeList.get(i).v_x = -GameCourt.SNAKE_VELOCITY;
				GameCourt.snakeList.get(i).v_y = 0;
			}
		}
			...
			if (Math.abs(x - GameCourt.snakeList.get(i).getX()) > 10 || Math.abs(y - GameCourt.snakeList.get(i).getY()) > 10) {
				GameCourt.snakeList.get(i).v_x = 0;
				GameCourt.snakeList.get(i).v_y = -GameCourt.SNAKE_VELOCITY;
			}
		}
			...
			GameCourt.snakeList.get(i).move();
			moveTails(i, x, y, tails, dir);
		}
	}
	
	/**
	 * Returns direction of GameObj
	 */	
	public Direction getDirection() {
		if (v_x == -(GameCourt.SNAKE_VELOCITY)) {
			return Direction.LEFT;
		}
		else if (v_x == GameCourt.SNAKE_VELOCITY) {
			return Direction.RIGHT;
		}
		else if (v_y == GameCourt.SNAKE_VELOCITY) {
	...
	/**
	 * Mutator methods for GameObj position
	 */
	...
	public boolean powerup(PowerUp obj) {
		return (pos_x + width >= obj.getX()
				&& pos_y + height >= obj.getY()
				&& obj.getX() + 10 >= pos_x 
				&& obj.getY() + 10 >= pos_y);
	}
	
	public boolean intersectTail() {
		boolean willIntersect = false;
		
		for (GameObj obj : GameCourt.snakeList) {
			if (pos_x + width >= obj.pos_x
				&& pos_y + height >= obj.pos_y
				&& obj.pos_x + obj.width >= pos_x 
				&& obj.pos_y + obj.height >= pos_y
				&& obj != GameCourt.snake
				&& obj != GameCourt.snakeList.get(1)) {
				return true;
			}
		}
